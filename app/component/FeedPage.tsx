  ...
  return (
    <div className="app-shell">
      ...
      <main className="feed-page">
        ...
        <div className="feed-list">{posts.map((post, index) => <FeedPostCard key={`${post.id}-${index}`} initialPost={post} onPostChanged={(updatedPost) => setPosts((currentPosts) => updatedPost.id ? currentPosts.map((currentPost) => currentPost.id === updatedPost.id ? updatedPost : currentPost) : currentPosts.filter((currentPost) => currentPost.id !== post.id))} onShare={(postToShare) => setSharePost(postToShare)} />)}</div>
      </main>
